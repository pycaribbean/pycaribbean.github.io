...
with open('PyCaribbean 2019 Submissions.json', 'r') as f:
    ...
    for talk in json_data:
        ...
        with open(file_name, 'w') as wf:
            ...
            talks.append({
                'avatar': talk['avatar'],
                'name': talk['name'],
                'url': '/talk/{0}.html'.format(talk['title'].replace(' ', '-').lower().replace('/', '-').replace(',', '').replace('(', '').replace(')', '')),
                'speaker_url': '/speaker/{0}.html'.format(talk['name'].replace(' ', '-').lower()),
                'title': talk['title'],
            ...
            wf.write(talk_template.format(
                ...
                talk['description'],
                talk['bio'],
                '/speaker/{0}.html'.format(talk['name'].replace(' ', '-').lower()),
            ),
